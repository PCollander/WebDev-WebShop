...
describe('Routes', () => {
  ...
  describe('handleRequest()', () => {
    ...
    describe('Method not allowed: POST /api/users/{id}', () => {
      it('should respond with "405 Method Not Allowed" when method is not allowed', async () => {
        const response = await chai
          .request(handleRequest)
          .post(`/api/users/${generateRandomString(10)}`)
          .set('Accept', contentType)
          .set('Authorization', `Basic ${adminCredentials}`);
    ...
    describe('Deleting users: DELETE /api/users/{id}', () => {
      ...
      it('cannot delete user itself', async () => {
        const { _id } = await User.findOne({ email: adminUser.email }).exec();
        const response = await chai
          .request(handleRequest)
          .delete(`${usersUrl}/${_id}`)
          .set('Accept', contentType)
        ...
        const dbUsers = await User.find({});
        expect(response).to.have.status(400);
        expect(dbUsers).to.be.lengthOf(allUsers.length);
      });
